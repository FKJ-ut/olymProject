                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($delegations as $delegation)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $delegation->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $delegation->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $delegation->tag }}</td>
                                    <!-- Manage Button -->
                                    ...
                                    <!-- Delete Button -->
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('delegations.destroy', $delegation->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this delegation?');">
                                            @csrf
                                            @method('DELETE')
